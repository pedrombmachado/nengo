...
class SimulationData(Mapping):
    ...
    used to access the parameters of objects in the model; for example,
    encoder values for an ensemble can be accessed via
    ``encoders = sim.data[my_ens].encoders``.

    This is like a view on the raw simulation data manipulated by the Simulator, which
    allows the raw simulation data to be optimized for speed while this provides a
